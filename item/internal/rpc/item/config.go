...
func loadConfig(ctx context.Context) (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("create item rpc config adapter: %w", err)
	}
	value, err := gcfg.NewWithAdapter(adapter).Get(ctx, ".")
	if err != nil {
		return Config{}, fmt.Errorf("read item rpc config: %w", err)
	}
	if value.IsEmpty() {
	...
	if err = value.Scan(&config); err != nil {
		return Config{}, fmt.Errorf("parse item rpc config: %w", err)
	}
	if err = config.validate(); err != nil {
		return Config{}, fmt.Errorf("validate item rpc config: %w", err)
	}
	return config, nil
}
