package cmd

import (
	"context"
	"time"

	"github.com/gogf/gf/v2/frame/g"
	"github.com/gogf/gf/v2/net/ghttp"
	"github.com/gogf/gf/v2/os/gcmd"
	obsconfig "one_adventure_observability_config"
	obslog "one_adventure_observability_log"
	metric "one_adventure_observability_metric"
	tracekit "one_adventure_observability_trace/trace"

	"one_adventure_servicekit/token"
	"ws_gateway/internal/controller/hello"
	servermanagerrpc "ws_gateway/internal/rpc/ws_gateway"
	wsproxy "ws_gateway/internal/websocket"
)

var (
	Main = gcmd.Command{
		Name:  "main",
		Usage: "main",
		Brief: "start http server",
		Func: func(ctx context.Context, parser *gcmd.Parser) (err error) {
			observability, err := obsconfig.Load(ctx, "trace.yaml")
			if err != nil {
				return err
			}
			shutdownTrace, err := tracekit.Init("ws_gateway", observability.TraceRuntime())
			if err != nil {
				return err
			}
			defer shutdownTrace(context.Background())
			shutdownMetric, err := metric.Init(observability.MetricRuntime())
			if err != nil {
				return err
			}
			defer shutdownMetric(context.Background())
			rpcServer, err := servermanagerrpc.New(ctx)
			if err != nil {
				return err
			}
			shutdownLog := obslog.Init("ws_gateway", rpcServer.InstanceID(), observability.LogRuntime())
			defer shutdownLog(context.Background())
			s := g.Server()
			proxy, err := wsproxy.NewProxy([]string{"etcd:2379"})
			if err != nil {
				return err
			}
			defer proxy.Close()
			if v, e := token.NewRS256VerifierFromFile("/app/key/public.pem", "one-adventure"); e == nil {
				wsproxy.SetVerifier(v)
			} else {
				return e
			}
			s.BindHandler("/ws_gateway/{server_id}/ws", proxy.Handle)
			s.Group("/", func(group *ghttp.RouterGroup) {
				group.Middleware(ghttp.MiddlewareHandlerResponse)
				group.Bind(
					hello.NewV1(),
				)
			})
			if err = s.Start(); err != nil {
				return err
			}
			if err = rpcServer.Start(); err != nil {
				_ = s.Shutdown()
				return err
			}
			ghttp.Wait()
			shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			return rpcServer.Shutdown(shutdownCtx)
		},
	}
)
